...
在目标代码生成部分，难点在于当前加载到了数组的哪个维度，以及计算下标i代表的实际偏移量。这用在数据结构部分已经提到的array_dims和array_ranges可以记录。此外，一个易错的地方在于如果alloc的值是一个指针的话（此时变量是一个指针的指针），那么需要再额外lw一次才能拿到我们想要的内容。
